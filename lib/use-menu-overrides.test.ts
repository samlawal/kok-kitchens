...
describe("createOverridesStore", () => {
  ...
  it("shares a single in-flight request across concurrent loads", async () => {
    ...
    resolveJson({ success: true, prices: { a: 1 }, statuses: {}, images: {} });
  ...
  it("caches a successful response and does not refetch", async () => {
    const fetcher = vi.fn(async () =>
      okResponse({ success: true, prices: { a: 2 }, statuses: {}, images: {} })
    );
    const store = createOverridesStore(fetcher as unknown as Fetcher);

    await store.load();
    expect(store.getCache()).toEqual({ prices: { a: 2 }, statuses: {}, images: {} });
    await store.load();
    expect(fetcher).toHaveBeenCalledTimes(1);
  });
  ...
  it("notifies subscribers on success", async () => {
    const fetcher = vi.fn(async () =>
      okResponse({ success: true, prices: {}, statuses: { x: "hidden" }, images: {} })
    );
    const store = createOverridesStore(fetcher as unknown as Fetcher);
    const cb = vi.fn();
    store.subscribe(cb);
  ...
  it("does not cache a non-success response and retries on next load", async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        okResponse({ success: true, prices: { a: 3 }, statuses: {}, images: {} })
      );
    const store = createOverridesStore(fetcher as unknown as Fetcher);

    const first = await store.load();
    expect(first).toEqual({ prices: {}, statuses: {}, images: {} });
  ...
  it("recovers from a rejected fetch without caching", async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        okResponse({ success: true, prices: { a: 4 }, statuses: {}, images: {} })
      );
    const store = createOverridesStore(fetcher as unknown as Fetcher);
